...
class SaleOrder(models.Model):
    ...
    def _get_reward_values_percentage_amount(self, program):
        ...
        if program.discount_apply_on == "cheapest_product":
        ...
        elif program.discount_apply_on in [
            "specific_products",
            "on_order",
            "all_products",
        ]:
            if program.discount_apply_on == "all_products":
                ...
                lines = lines.filtered(lambda x: x.product_id in program_product_ids)
            elif program.discount_apply_on == "specific_products":
                # We should not exclude reward line that offer this product
                # since we need to offer only the discount on the real paid product
            ...
            for line in lines:
                ...
                if discount_line_amount:
                    ...
                    if line.tax_id in reward_dict:
                        ...
                        if program.discount_apply_on == "all_products":
                            name = _(
                                "Discount: %s - On targeted products" % program.name
                            )
                        reward_dict[line.tax_id] = {
                            "name": name,
                            "product_id": program.discount_line_product_id.id,
        ...
        # If there is a max amount for discount,
        # we might have to limit some discount lines or completely remove some lines
