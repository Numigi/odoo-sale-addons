...
class TestProjectBilling(TestCommonSaleTimesheetNoChart):
    ...
    def test_make_billable_at_task_rate(self):
        ...
        self.assertEqual(
            self.project_non_billable.billable_type,
            "no",
            "The project should still be non billable",
        )
        self.assertEqual(
            wizard.partner_id,
        ...
        self.assertEqual(
            self.project_non_billable.billable_type,
            "task_rate",
            "The project should be 'task rate' billable",
        )
        self.assertEqual(
    ...
    def test_make_billable_at_employee_rate(self):
        ...
        self.assertEqual(
            self.project_non_billable.billable_type,
            "employee_rate",
            "The project should be 'employee rate' billable",
        )
        self.assertEqual(
            sale_order.partner_id,
            self.project_non_billable.partner_id,
    ...
    def test_billing_employee_rate(self):
        ...
        self.assertFalse(
            task.sale_line_id,
            "Task created in a project billed on 'employee rate' should not be linked to a SOL",
        )
        self.assertEqual(
            task.partner_id,
        ...
        subtask = Task.with_context(
            default_project_id=self.project_employee_rate.subtask_project_id.id
        ).create(
            {
        ...
        self.assertEqual(
            ...
            "Task moved in a task rate billable project",
        )
        self.assertEqual(
            task.partner_id,
            task.project_id.partner_id,
        ...
        self.assertEquals(
            ...
            "The timesheet should be linked to SOL on the project, as no entry for TDE in project map and no SOL on task",
        )
    ...
    def test_billing_task_rate(self):
        ...
        self.assertEqual(
            task.sale_line_id,
            self.project_task_rate.sale_line_id,
            "Task created in a project billed on 'task rate' should be linked to a SOL of the project",
        )
        self.assertEqual(
            task.partner_id,
            task.project_id.partner_id,
        ...
        # create a subtask
        subtask = Task.with_context(
            default_project_id=self.project_task_rate.subtask_project_id.id
        ).create(
            {
                "name": "first subtask task",
                "parent_id": task.id,
        ...
        self.assertEqual(
            subtask.billable_type,
            "task_rate",
            "Subtask in a non billable project with a so line set is task rate billable",
        )
        self.assertEqual(
            subtask.project_id.billable_type,
            "no",
        ...
        self.assertEqual(
            ...
            "The timesheet should be linked to SOL as the task even in a non billable project",
        )
        ...
        self.assertEqual(
            task.billable_type,
            "employee_rate",
            "Task moved in project 'employee rate' should be billed at employee rate",
        )
        self.assertFalse(
            task.sale_line_id, "Task moved in a employee rate billable project have empty so line"
        )
        self.assertEqual(
            task.partner_id,
            task.project_id.partner_id,
        ...
        self.assertEqual(
            subtask.billable_type,
            "employee_rate",
            "subtask moved in project 'employee rate' should be billed at employee rate",
        )
        self.assertFalse(
            subtask.sale_line_id,
            "Subask moved in a employee rate billable project have empty so line",
        )
        self.assertEqual(
            subtask.partner_id,
            task.project_id.partner_id,
            "Subask created in a project billed on 'employee rate' should have the same customer as the one from the project",
        )
