...
class TestSaleService(TestCommonSaleTimesheetNoChart):
    ...
    def test_task_so_line_assignation(self):
        ...
        self.assertEqual(
            ...
            "Old timesheet are not modified when changing the task SO line",
        )
    ...
    def test_billable_task_and_subtask(self):
        ...
        self.assertEqual(
            ...
            task.sale_line_id,
            "By, default, a child task should have the same SO line than its mother",
        )
        self.assertEqual(
            task2.sale_line_id,
            project.sale_line_id,
            "A new task in a billable project should have the same SO line than its project",
        )
        self.assertEqual(
            task2.partner_id,
        ...
        self.assertEqual(
            subtask.sale_line_id,
            task.sale_line_id,
            "A child task should always have the same SO line than its mother, even when changing project",
        )
    ...
    def test_copy_billable_project_and_task(self):
        ...
        self.assertFalse(
            project_copy.sale_line_id, "Duplicatinga project should erase its Sale line"
        )
        self.assertFalse(
            project_copy.sale_order_id, "Duplicatinga project should erase its Sale order"
        )
        self.assertEqual(
            project_copy.billable_type,
            "no",
            "Duplicatinga project should reset its billable type to none billable",
        )
        self.assertEqual(
            len(project.tasks),
        ...
        task_copy = task.copy()
        self.assertEqual(
            task.sale_line_id, task_copy.sale_line_id, "Duplicatinga task should keep its Sale line"
        )
