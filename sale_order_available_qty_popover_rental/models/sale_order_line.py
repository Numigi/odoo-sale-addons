...
class SaleOrderLine(models.Model):
    ...
    def _get_available_qty_for_popover_sale(self):
        stock_quant_domain = [('product_id', '=', self.product_id.id),
    ...
    def _get_location_for_sale(self):
        location_domain = [('usage', '=', 'internal'), ('is_rental_customer_location', '=', False)]
        return self.env['stock.location'].search(location_domain).ids

    def _get_location_for_rental(self):
        location_domain = [('usage', '=', 'internal'), ('is_rental_customer_location', '=', True)]
        return self.env['stock.location'].search(location_domain).ids
