...
class CommissionTarget(models.Model):
    ...
    already_generated = fields.Monetary(
        compute="_compute_already_generated", store=True
    )
    left_to_generate = fields.Monetary(compute="_compute_left_to_generate",
                                       store=True)
    ...
    def _compute_already_generated(self):
        for target in self:
            target.already_generated = sum(
                line.amount for line in target.payroll_line_ids
            )
