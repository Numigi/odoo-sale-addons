...
class SaleWarranty(models.Model):
    ...
    extension_start_date = fields.Date(tracking=True, copy=False)
    extension_expiry_date = fields.Date(tracking=True, copy=False)
    extension_template_id = fields.Many2one(related="type_id.extension_template_id")
    extension_subscription_id = fields.Many2one(
        "sale.subscription", "Extension Contract", ondelete="restrict"
    )
    ...
    def _onchange_activation_date_set_extension_dates(self):
        ...
        if self.activation_date and self.type_id and self.use_warranty_extension:
            ...
            extension_expiry = extension_start + relativedelta(
                months=self.type_id.extension_duration_in_months
            )
    ...
    def _check_extension_start_prior_to_expiry(self):
        for warranty in self:
            if (
                warranty.extension_start_date
                and warranty.extension_expiry_date
                and warranty.extension_start_date > warranty.extension_expiry_date
            ):
    ...
    def find_warranties_to_set_expired(self):
        ...
        return expired_warranties.filtered(
            lambda w: (
                not w.use_warranty_extension
                or not w.extension_expiry_date
                or w.extension_expiry_date < today
            )
        )
    ...
    def _should_be_extended(self):
        return self.type_id.use_warranty_extension and bool(
            self._get_active_extension_subscription()
        )
    ...
    def _get_extension_expiry_date(self):
        return (
            self._get_extension_start_date()
