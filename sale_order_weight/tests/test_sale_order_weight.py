...
class TestSaleOrderWeight(SavepointCase):
    @classmethod
    def setUpClass(cls):
        super().setUpClass()
        cls.unit_uom = cls.env.ref("uom.product_uom_unit")
        cls.dozen_uom = cls.env.ref("uom.product_uom_dozen")
        cls.pound_uom = cls.env.ref("uom.product_uom_lb")
        cls.kgm_uom = cls.env.ref("uom.product_uom_kgm")
    ...
    def _assert_weight(self, so_weight, expected_weight, uom=None):
        ...
        self.assertEqual(
            float_compare(
                so_weight, expected_weight, precision_digits=precision_rounding
            ),
            0,
        )
    ...
    def test_product_with_weight_in_other_uom(self):
        ...
        self._assert_weight(
            so.weight_in_lb, 2 * 1 * (200 / 1000) * 2.20462, uom=self.lb_uom
        )
    ...
    def test_sale_order_line_with_custom_uom(self):
        ...
        self._assert_weight(
            so.weight_in_lb, 2 * 12 * (200 / 1000) * 2.20462, uom=self.lb_uom
        )
    ...
    def test_sale_order_line_update(self):
        ...
        self._assert_weight(
            so.weight_in_lb, 4 * 1 * (200 / 1000) * 2.20462, uom=self.lb_uom
        )
    ...
    def test_multiple_sale_order_lines(self):
        ...
        self._assert_weight(
            so.weight_in_lb,
            ((2 * 1 * 100) + (4 * 1 * (200 / 1000)) * 2.20462 + (6 * 12 * 100)),
            uom=self.lb_uom,
        )
