...
class SaleOrderLine(models.Model):
    ...
    qty_reserved = fields.Float("Qty Reserved", compute="_get_reserved_qty")
    virtual_available_at_date = fields.Float(
        compute="_compute_qty_at_date_2", digits="Product Unit of Measure"
    )
    ...
    def _get_reserved_qty(self):
        for rec in self:
            rec.qty_reserved = rec.qty_available_today - rec.free_qty_today

    @api.depends("free_qty_today", "qty_to_deliver")
    def _compute_qty_available_color(self):
        for rec in self:
            color = "text-danger"
            if rec.free_qty_today and rec.qty_to_deliver <= rec.free_qty_today:
                color = "text-success"
